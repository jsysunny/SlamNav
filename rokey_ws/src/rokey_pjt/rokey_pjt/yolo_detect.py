...
class DetectAllObjectsWithDistance(Node):
    def __init__(self):
        ...
        self.create_subscription(Image, '/robot2/oakd/stereo/image_raw', self.depth_callback, 10)
    ...
    def process_image(self):
        ...
        for idx in range(len(boxes)):
            ...
            if distance_m <DANGER_THRESHOLD and class_name =="prohibition":
                ...
                print(f"Danger {distance_m:.2f}")
                danger_msg = Bool()
                danger_msg.data = self.danger_detected
                self.danger_pub.publish(danger_msg)
                self.get_logger().info("danger 메세지 수신")
                # 다음 루프 전에 danger 상태 초기화
                self.danger_detected = False
